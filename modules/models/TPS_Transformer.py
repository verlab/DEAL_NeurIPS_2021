...
class TPS_Transformer(torch.nn.Module):
    def __init__(self, ctrlshape, useResNet): 
        ...
        self.T = SpatialTransformer(transform = 'cartesian', identity= True, resolution=3, featureMap=True, 
                    SIFTscale = 8.0).to(self.device)
        ...
        self.loc = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Dropout(0.1),
            torch.nn.Linear(512, self.nparam*2),
            torch.nn.Tanh()
        ).to(self.device)
    ...
    def forward(self, polargrid, theta, imgs, imgIDs):
        ...
        if self.useResNet:
          ...
          #polargrid = (polargrid + 1.0)/2
          #normalize each elem in batch to [0,1]
          kfactor = 0.3
...
class BaseModel(torch.nn.Module):
    def __init__(self, useResNet = False, pretrainedResNet = True, nchannels = 1):
        ...
        self.tps_transform = TPS_Transformer(ctrlshape=(8,8), useResNet=useResNet).to(self.device)
